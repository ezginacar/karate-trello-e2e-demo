...
Feature: E2E Case Study - Complete Trello Workflow
  ...
  Scenario: Complete E2E Test - Create Organization, Board, List, Cards, Update and Cleanup
    ...
    * cleanup.registerBoard('#(board.result.id)')
    ...
    * cleanup.registerList('#(list.result.id)')
    ...
    * cleanup.registerCard('#(card1.result.id)')
    ...
    * cleanup.registerCard('#(card2.result.id)')
    ...
    * def cleanup = 
    ...
    function() {
      if(scenario.status == 'failed') {
        ...
        cleanup.performHierarchicalCleanup();
      }
    }
    """
    * call cleanup
